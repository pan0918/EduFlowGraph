"""Lightweight learner-portrait metadata.

The learner profile is intentionally small: each of the two portrait models is a single
...
2. context_model  — Current learning situation (scene adaptation)

Skill-adaptation evidence is a separate state owned by the Skill workbench. It is
used for Skill retrieval/reranking, but is not part of the learner portrait.
"""
...
EPISODE_MODELS = ("learner_model",)
# Model refreshed lightly on every turn (time-sensitive situation).
TURN_MODEL = "context_model"

SKILL_ADAPTATION_NAME = "skill_adaptation"
SKILL_ADAPTATION_META = {
    "label": "技能适配证据",
    "subtitle": "Skill 选择 — 个性化重排与过滤依据",
    "description": "描述更适合或应避免的 Skill 类型、认知负荷、教学节奏与验证偏好，不保存具体教学程序；它用于 Skill 检索和重排，不属于用户画像。",
    "icon": "compass",
    "budget": 300,
}
...
def skill_adaptation_budget() -> int:
    return int(SKILL_ADAPTATION_META["budget"])
